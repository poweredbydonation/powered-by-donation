...
export async function POST(
  ...
  try {
    ...
    // Verify the service request exists and user is the donor
    const { data: serviceRequest, error: fetchError } = await supabase
      .from('service_requests')
      .select('*')
      .eq('id', requestId)
      .eq('donor_id', user.id)
      .in('workflow_status', ['service_requested', 'service_request_accepted'])
      .single()
    ...
    // For cancel, we set to appropriate timeout status based on current state
    const timeoutStatus = serviceRequest.workflow_status === 'service_requested' 
    ...
    if (transitionError) {
      return NextResponse.json(
        { error: 'Failed to cancel request', details: transitionError.message },
        { status: 500 }
      )
    }
