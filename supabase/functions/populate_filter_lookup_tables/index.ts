...
function parseOperatingCountries(countriesString) {
  if (!countriesString || typeof countriesString !== 'string') return [];
  return countriesString.split(/[,;|]/).map((code)=>code.trim()).filter((code)=>code.length > 0).map((code)=>getCountryName(code)).filter((name)=>name !== null).filter((name, index, arr)=>arr.indexOf(name) === index).sort();
}
const corsHeaders = {
...
async function processJustGivingData(supabaseClient, justgivingOrgs) {
...
async function processEveryOrgData(supabaseClient, everyorgOrgs) {
...
async function updateLookupTable(supabaseClient, tableName, fieldName, dataCount) {
  if (dataCount.size === 0) {
    console.log(`No data to update for ${tableName}`);
    return;
  }
  // Clear existing data
  const { error: deleteError } = await supabaseClient.from(tableName).delete().neq('id', 0);
  if (deleteError) {
  ...
  const { error: insertError } = await supabaseClient.from(tableName).insert(insertData);
  if (insertError) {
