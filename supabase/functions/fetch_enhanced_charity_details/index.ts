...
async function fetchCharityDetails(charityId, apiKey) {
  const urlVariations = [
  ...
  const headerVariations = [
    ...
    {
      'Content-Type': 'application/json',
      'User-Agent': 'PoweredByDonation/1.0'
    }
  ];
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: charities, error: fetchError } = await supabase.from('organization_cache').select('external_id, name, api_fetch_attempts').eq('platform', 'justgiving').is('enhanced_data_fetched_at', null).lt('api_fetch_attempts', 3).eq('is_active', true).limit(20);
    if (fetchError) {
    ...
    for (const charity of charities){
      ...
      try {
        ...
        const { error: updateError } = await supabase.from('organization_cache').update({
          // Address information
        ...
        }).eq('platform', 'justgiving').eq('external_id', charity.external_id);
        if (updateError) {
        ...
        await supabase.from('organization_cache').update({
