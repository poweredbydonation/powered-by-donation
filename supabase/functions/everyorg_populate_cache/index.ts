...
function prepareNonprofitForCache(nonprofit: EveryOrgNonprofit): any {
...
async function batchUpsertNonprofits(nonprofits: any[], supabase: any): Promise<{ success: number; errors: number }> {
  try {
    ...
    if (error) {
      ...
      return { success: 0, errors: nonprofits.length };
    } else {
      console.log(`Successfully batch cached ${nonprofits.length} nonprofits`);
      return { success: nonprofits.length, errors: 0 };
    }
  } catch (error) {
...
async function getProcessingState(supabase: any): Promise<{ causeIndex: number; page: number; cause?: string }> {
  try {
    const { data, error } = await supabase
      .from('organization_cache')
      .select('description')
      .eq('platform', 'everyorg')
      .eq('slug', 'progress-tracker')
      .single();
      
    if (error || !data) {
      console.log('No existing progress tracker found, starting from beginning');
      return { causeIndex: 0, page: 1 };
    }
...
async function saveProcessingState(causeIndex: number, page: number, cause: string, supabase: any, stats?: any): Promise<void> {
  try {
    const progressData = {
      causeIndex,
      page,
...
serve(async (req) => {
  ...
  try {
    ...
    const ALL_CAUSES = [
      ...
      'culture', 'visual-art', 'dance', 'filmandtv', 'museums', 'music', 'theater',
      'education', 'libraries', 'scholarships',
      'environment', 'agriculture', 'climate', 'conservation', 'oceans', 'parks',
      'health', 'autism', 'cancer', 'disease', 'mental-health', 'womens-health',
      'humans', 'adoption', 'entrepreneurship', 'food-security', 'housing', 'immigrants', 
      'indigenous-peoples', 'poverty', 'refugees', 'seniors', 'veterans', 'water', 'youth',
      'justice', 'aapi-led', 'black-led', 'disabilities', 'freepress', 'gender-equality', 
      'indigenous-led', 'latine-led', 'legal', 'lgbt', 'racial-justice', 'votingrights', 'women-led',
      'religion', 'buddhism', 'christianity', 'hinduism', 'islam', 'judaism',
    ...
    while (causeIndex < ALL_CAUSES.length && 
           Date.now() - startTime < MAX_RUNTIME_MS && 
           requestCount < MAX_REQUESTS_PER_RUN) {
