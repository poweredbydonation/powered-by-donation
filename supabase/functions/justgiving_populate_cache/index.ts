...
async function searchJustGivingByName(charityName, apiKey) {
...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: charityNames, error: fetchError } = await supabase.from('JustGivingCharityNames').select('Charity_Name, query_datetime, charity_id, add_to_cache_date').is('query_datetime', null) // Not yet queried
    .order('Charity_Name').limit(BATCH_SIZE);
    if (fetchError) {
    ...
    for (const charityNameRow of charityNames){
      ...
      try {
        ...
        await supabase.from('JustGivingCharityNames').update({
        ...
        const { data: existingCharity } = await supabase.from('organization_cache').select('external_id').eq('platform', 'justgiving').eq('external_id', selectedCharity.charityId.toString()).single();
        if (existingCharity) {
          alreadyCached++;
          console.log(`✅ Charity already cached: "${selectedCharity.name}"`);
          // Update add_to_cache_date since it's already cached
          await supabase.from('JustGivingCharityNames').update({
            add_to_cache_date: new Date().toISOString()
          }).eq('Charity_Name', charityName);
          continue;
        }
        ...
        const { error: insertError } = await supabase.from('organization_cache').upsert({
          platform: 'justgiving',
